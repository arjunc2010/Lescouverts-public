...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminTime&amp;action=index" method="post" class="form" id="frmTimeCustom">
	...
	<p>
		...
		<?php TimeWidget::hour(null, 'start_hour', 'start_hour', 'select pps', array(), array('time_format' => $tpl['option_arr']['time_format'])); ?>
		<?php TimeWidget::minute(null, 'start_minute', 'start_minute', 'select w50 pps'); ?>
	</p>
	<p>
		<label class="title"><?php echo $RB_LANG['time_to']; ?></label>
		<?php TimeWidget::hour(null, 'end_hour', 'end_hour', 'select pps', array(), array('time_format' => $tpl['option_arr']['time_format'])); ?>
		...
		if ($count > 0)
		{
			...
			for ($i = 0; $i < $count; $i++)
			{
				...
				<tr class="<?php echo $i % 2 === 0 ? 'odd' : 'even'; ?>">
					...
					<td><a class="icon icon-delete" rel="<?php echo $tpl['date_arr'][$i]['id']; ?>" href="<?php echo  $_SERVER['PHP_SELF']; ?>?controller=pjAdminTime&amp;action=delete&amp;id=<?php echo $tpl['date_arr'][$i]['id']; ?>"><?php echo $RB_LANG['_delete']; ?></a></td>
				</tr>
				<?php
			}
			?>
