				...
				if ($count > 0)
				{
					...
					for ($i = 0; $i < $count; $i++)
					{
						...
						<tr class="<?php echo $i % 2 === 0 ? 'odd' : 'even'; ?> pointer {id: '<?php echo $tpl['arr'][$i]['id']; ?>'}">
							...
							<td><a class="icon icon-delete" rel="<?php echo $tpl['arr'][$i]['id']; ?>" href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminBookings&amp;action=delete&amp;id=<?php echo $tpl['arr'][$i]['id']; ?>"><?php echo $RB_LANG['_delete']; ?></a></td>
						</tr>
						<?php
					}
					?>
